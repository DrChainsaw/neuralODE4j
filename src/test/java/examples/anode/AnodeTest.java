...
public class AnodeTest {
    ...
    /**
     * Test claims of section 3 in paper part 1: Verify that the function {g(1) = -1, g(-1) = 1} can not be learned by a
     * normal neural ODE function
     */
    @Test
    public void failToSolveIntersectMapping() {
        ...
        final NeuralNetConfiguration.Builder globalConf = new NeuralNetConfiguration.Builder()
                ...
                .seed(666)
                .updater(new Adam(0.07));

        final long nrofHidden = 8;
        final ComputationGraph graph = new ComputationGraph(globalConf
                .graphBuilder()
                .addInputs("input")
                .setInputTypes(InputType.feedForward(1))

                .addVertex("ode",
                        new OdeVertex.Builder(globalConf, "dense0",
        ...
        assertEquals("Incorrect solution!", 0, graph.outputSingle(input).getDouble(0), 1e-4);
        assertEquals("Incorrect solution!", 0, graph.outputSingle(input.neg()).getDouble(0), 1e-4);
    }

    private DataSet create1DDataSet(int batchSize, INDArray inputProto) {
        final INDArray input = Nd4j.repeat(inputProto.reshape(1), batchSize);
