...
public class ShapeMatchVertexTest {
    ...
    public void doForward() {
        ...
        final GraphVertex vertex = new ShapeMatchVertex(null, "test", 1,
                new ElementWiseVertex(null, "dummy", 1, ElementWiseVertex.Op.Add));
    ...
    /**
     * Test doBackward with a {@link org.deeplearning4j.nn.graph.vertex.impl.MergeVertex}
     */
    @Test
    public void doBackward() {
        final INDArray eps = Nd4j.arange(2*3*4).reshape(2,3,4);
        final GraphVertex vertex = new ShapeMatchVertex(null, "test", 1,
                new ElementWiseVertex(null, "test-vertex", 1, ElementWiseVertex.Op.Average));

        vertex.setEpsilon(eps);
        vertex.setInputs(eps, eps, Nd4j.scalar(1));
        vertex.doForward(true ,LayerWorkspaceMgr.noWorkspacesImmutable()); // Needed to set inputs of MergeVertex
