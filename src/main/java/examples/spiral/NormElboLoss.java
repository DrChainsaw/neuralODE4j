...
public class NormElboLoss implements ILossFunction{
    ...
    private final ActivationListener qz0MeanListener;
    private final ActivationListener qz0LogNoiseVarListener;

    public NormElboLoss(double mean, double noiseSigma, ActivationListener qz0MeanListener, ActivationListener qz0LogNoiseVarListener) {
        this.logNoiseVar = 2*Math.log(noiseSigma);
        this.logMean = Math.log(mean);
        this.qz0MeanListener = qz0MeanListener;
        this.qz0LogNoiseVarListener = qz0LogNoiseVarListener;
    }
    ...
    private INDArray scoreArray(INDArray labels, INDArray preOutput, IActivation activationFn, INDArray mask) {
        ...
        final int[] sumDims = new int[preOutput.rank()-1];
        for(int i = 0; i < sumDims.length; i++) {
            sumDims[i] = i+1;
        }

        final INDArray logPx = logNormalPdf(labels, output).sum(sumDims);
        final INDArray analyticKl = normalKl().sum(1);
    ...
    private INDArray normalKl( ) {
        final INDArray mu1 = qz0MeanListener.lastActivation().dup();
        final double mu2 = 0;
        final INDArray lv1 = qz0LogNoiseVarListener.lastActivation().dup();
