 ...
 */
@Data
public class ScaleUpLossGradPreProcessor implements InputPreProcessor {

    private final InputPreProcessor inputPreProcessor;

    public ScaleUpLossGradPreProcessor(@JsonProperty("inputPreProcessor") InputPreProcessor inputPreProcessor) {
    ...
    public INDArray backprop(INDArray output, int miniBatchSize, LayerWorkspaceMgr workspaceMgr) {
        double scaling = output.size(0) / (double)miniBatchSize;
        return inputPreProcessor.backprop(output.muli(scaling), miniBatchSize, workspaceMgr);
    }

    @Override
    public InputPreProcessor clone() {
        return new ScaleUpLossGradPreProcessor(inputPreProcessor.clone());
    }
